...
political_office.add_argument('officetype', type=str, help='Please enter your officetype', required=True)
...
class Office(Resource):
  def post(self):
    ...
    if not officetype.isalpha():
      return{"status": 400, "message":"Office type should be alphabet"}, 400

    if not name.isalpha():
      return {"status":400, "message": "Name type should be alphabet"}, 400

    for office in offices:
      if officetype in office.values() or name in office.values():
        return {"status": 409, "message": "either office or name already exists"}, 409
...
class SingleOffice(Resource):
  def get(self,id):
    ...
    if not single_office.single_office(id):
      return {"status":404," message": "No office availabe now please try later"}, 404
...
class GetAllOffice(Resource):
  def get(self):
    ...
    if not office.all_office():
      return {"status":404, "message": "No offices availabe now please try later"}, 404
