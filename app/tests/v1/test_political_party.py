...
class BaseTest(unittest.TestCase):
  def setUp(self):
    ...
    self.test_patch_political_party_with_missing_fields = {
        "name": "", 
        "headquateraddress" : "",
        "logo": "", 
    }
  ...
  def test_patch_political_party_with_missing_fields(self):
    register = self.client.patch('/party/1', data=self.test_patch_political_party_with_missing_fields, content_type='multipart/form-data')
    self.assertEqual(register.status_code, 400)
